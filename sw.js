const CACHE='treino-feminino-prod-v6';
const ASSETS=['./index.html','./manifest.json','./config-tools.js','./history-tools.js','./workout-input-tools.js','./version-tools.js','./block-workout-tools.js','./female-enhancements.js','./profile-tools.js','./lances-theme.js','./lances-tech-logo.png'];
...
self.addEventListener('fetch',event=>{
 ...
 if(url.pathname.endsWith('/index.html')||url.pathname.endsWith('/')){
  event.respondWith(fetch(event.request,{cache:'no-store'}).then(async response=>new Response(injectTools(await response.text()),{headers:{'Content-Type':'text/html; charset=utf-8','Cache-Control':'no-store'}})).catch(()=>caches.match('./index.html').then(async response=>response?new Response(injectTools(await response.text()),{headers:{'Content-Type':'text/html; charset=utf-8'}}):new Response('Offline',{status:503}))));
  return;
 }
